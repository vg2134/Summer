...
from lf.stn.gridgen import AffineGridGen, PerspectiveGridGen, GridGen
import numpy as np
from utils import transformation_utils
from lf.lf_cnn import makeCnn
from lf.fast_patch_view import get_patches
...
class LineFollower(nn.Module):
    ...
    def forward(self, image, positions, steps=None, all_positions=[], reset_interval=-1, randomize=False,
        ...
        for i in range(steps):
            ...
            if resampled is None and i == 0:
                ...
                # in the future
                resampled = Variable(torch.zeros(crop_window.size(0), 3, 32, 32).type_as(image.data),
                                     requires_grad=False)
