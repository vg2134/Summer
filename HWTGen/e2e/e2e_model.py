...
from e2e import handwriting_alignment_loss
...
class E2EModel(nn.Module):
    ...
    # def __init__(self, sol, lf, hw, dtype=torch.FloatTensor):
    ...
    def forward(self, x, use_full_img=True, accpet_threshold=0.1, volatile=True, gt_lines=None, idx_to_char=None):
        with torch.no_grad():
            sol_img = Variable(x['resized_img'].type(self.dtype), requires_grad=False, volatile=volatile).cuda()

            if use_full_img:
                img = Variable(x['full_img'].type(self.dtype), requires_grad=False, volatile=volatile).cuda()
            ...
            # print(sorted_start)
            # print(sorted_start[0, 1, 0].data.cpu())
            min_threshold = sorted_start[0, 1, 0].data.cpu()
            ...
            for p in range(0, min(positions.size(0), np.inf), p_interval):
                ...
                print(type(expand_img))
                print(type(sub_positions[0]))
