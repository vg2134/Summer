...
def decode_handwriting(out, idx_to_char):
    ...
    for i in range(hw_out.shape[0]):
        ...
        for idx, value in enumerate(pred):
            pred_str = string_utils.label2str_single(value, idx_to_char, False)
            raw_pred_str = string_utils.label2str_single(raw_pred[idx], idx_to_char, True)
            list_of_pred[idx].append(pred_str)
            list_of_raw_pred[idx].append(raw_pred_str)
